-- FIXED COMPLETE SYSTEM FIX
-- This script handles existing table structures and fixes ALL issues
...
-- ===========================================
-- STEP 3: CREATE/UPDATE TABLES WITH CORRECT STRUCTURE
-- ===========================================
...
-- Students table (create if not exists, handle existing structure)
...
-- If students table exists but has different structure, add missing columns
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'students' AND column_name = 'university') THEN
        ALTER TABLE public.students ADD COLUMN university TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'students' AND column_name = 'major') THEN
        ALTER TABLE public.students ADD COLUMN major TEXT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'students' AND column_name = 'skills') THEN
        ALTER TABLE public.students ADD COLUMN skills TEXT[];
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'students' AND column_name = 'portfolio_url') THEN
        ALTER TABLE public.students ADD COLUMN portfolio_url TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'students' AND column_name = 'is_verified') THEN
        ALTER TABLE public.students ADD COLUMN is_verified BOOLEAN DEFAULT FALSE;
    END IF;
...
-- Buyers table (create if not exists, handle existing structure)
...
-- If buyers table exists but has different structure, add missing columns
DO $$ 
BEGIN
    -- Add user_id column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'user_id') THEN
        ALTER TABLE public.buyers ADD COLUMN user_id UUID REFERENCES public.users(id) ON DELETE CASCADE;
    END IF;
    
    -- Add other missing columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'company_name') THEN
        ALTER TABLE public.buyers ADD COLUMN company_name TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'industry') THEN
        ALTER TABLE public.buyers ADD COLUMN industry TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'company_size') THEN
        ALTER TABLE public.buyers ADD COLUMN company_size TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'website') THEN
        ALTER TABLE public.buyers ADD COLUMN website TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'created_at') THEN
        ALTER TABLE public.buyers ADD COLUMN created_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'buyers' AND column_name = 'updated_at') THEN
        ALTER TABLE public.buyers ADD COLUMN updated_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
END $$;
...
-- Orders table (create if not exists)
...
-- Project applications table (create if not exists)
...
-- Chat threads table (create if not exists)
...
-- Insert sample student profile (only if user_id column exists)
DO $$ 
BEGIN
...
-- Insert sample buyer profile (only if user_id column exists)
DO $$ 
BEGIN
...
-- ===========================================
SELECT 'FIXED SYSTEM FIX APPLIED SUCCESSFULLY! All dashboards, orders, and messaging should now work perfectly.' as message;
